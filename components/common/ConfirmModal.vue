  ...
  <UModal :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)">
    <div class="p-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg">
      <div class="flex items-start gap-4">
        <div class="p-2 rounded-full" :class="danger ? 'bg-red-500/10 text-red-500' : 'bg-gray-500/10 text-gray-400 dark:text-gray-500'">
          <UIcon :name="danger ? 'i-heroicons-exclamation-triangle' : 'i-heroicons-information-circle'" class="text-2xl" />
        </div>
        <div class="flex-1">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ title }}</h3>
          <p class="text-gray-600 dark:text-gray-400 text-sm mb-6">{{ description }}</p>
