<template>
  <Transition name="slide">
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-end">
      <!-- Backdrop -->
      <div 
        class="absolute inset-0 bg-black/50 transition-opacity"
        @click="$emit('update:modelValue', false)"
      />
      
      <!-- Panel -->
      <div class="relative w-full max-w-2xl h-full bg-gradient-dark text-white shadow-2xl overflow-y-auto">
        <!-- Background pattern overlay -->
        <div class="absolute inset-0 bg-pattern opacity-[0.015] pointer-events-none"></div>
        
        <!-- Header -->
        <div class="sticky top-0 z-10 bg-white/5 backdrop-blur-xl border-b border-white/10 px-6 py-4 relative">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-2xl font-bold">Creative Details</h2>
            
            <div class="flex items-center gap-2">
              <button 
                @click="$emit('update:modelValue', false)"
                class="w-9 h-9 hover:bg-white/10 rounded-lg flex items-center justify-center transition-colors"
              >
                <UIcon name="i-heroicons-x-mark" class="text-xl text-slate-400" />
              </button>
            </div>
          </div>
        </div>

        <!-- Content -->
        <div v-if="creative" class="px-6 py-6 relative">
          <h1 class="text-2xl font-semibold mb-6">{{ creative.name }}</h1>

          <!-- Creative Preview -->
          <div v-if="creative.asset" class="mb-6">
            <div class="aspect-video bg-gray-800 rounded-lg overflow-hidden">
              <img 
                v-if="creative.asset.file_type === 'image'"
                :src="creative.asset.public_url"
                :alt="creative.name"
                class="w-full h-full object-cover"
              />
              <div v-else class="w-full h-full flex items-center justify-center">
                <UIcon name="i-heroicons-film" class="text-6xl text-gray-400" />
              </div>
            </div>
          </div>

          <!-- Creative Details -->
          <div class="space-y-6">
            <!-- Name -->
            <div class="flex items-start gap-4">
              <label class="w-32 text-sm text-gray-400 pt-2">Name</label>
              <div class="flex-1">
                <input
                  type="text"
                  :value="creative.name"
                  @blur="(e) => updateField('name', (e.target as HTMLInputElement).value)"
                  class="w-full px-3 py-2 bg-gray-900/50 border border-gray-800 rounded-lg text-sm text-gray-300 focus:outline-none focus:border-gray-700"
                />
              </div>
            </div>

            <!-- Status -->
            <div class="flex items-start gap-4">
              <label class="w-32 text-sm text-gray-400 pt-2">Status</label>
              <div class="flex-1">
                <select
                  :value="creative.status"
                  @change="(e) => updateField('status', (e.target as HTMLSelectElement).value)"
                  class="w-full px-3 py-2 bg-gray-900/50 border border-gray-800 rounded-lg text-sm text-gray-300 focus:outline-none focus:border-gray-700 cursor-pointer"
                >
                  <option value="draft">Draft</option>
                  <option value="in_review">In Review</option>
                  <option value="approved">Approved</option>
                  <option value="rejected">Rejected</option>
                  <option value="live">Live</option>
                  <option value="paused">Paused</option>
                </select>
              </div>
            </div>

            <!-- Format -->
            <div class="flex items-start gap-4">
              <label class="w-32 text-sm text-gray-400 pt-2">Format</label>
              <div class="flex-1">
                <select
                  :value="creative.format"
                  @change="(e) => updateField('format', (e.target as HTMLSelectElement).value)"
                  class="w-full px-3 py-2 bg-gray-900/50 border border-gray-800 rounded-lg text-sm text-gray-300 focus:outline-none focus:border-gray-700 cursor-pointer"
                >
                  <option value="single_image">Single Image</option>
                  <option value="carousel">Carousel</option>
                  <option value="video">Video</option>
                  <option value="collection">Collection</option>
                </select>
              </div>
            </div>

            <!-- Budget -->
            <div class="grid grid-cols-2 gap-4">
              <div class="flex items-start gap-4">
                <label class="w-32 text-sm text-gray-400 pt-2">Daily Budget</label>
                <div class="flex-1">
                  <input
                    type="number"
                    :value="creative.daily_budget || ''"
                    @blur="(e) => updateField('daily_budget', parseFloat((e.target as HTMLInputElement).value) || null)"
                    placeholder="0.00"
                    step="0.01"
                    class="w-full px-3 py-2 bg-gray-900/50 border border-gray-800 rounded-lg text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-gray-700"
                  />
                </div>
              </div>

              <div class="flex items-start gap-4">
                <label class="w-32 text-sm text-gray-400 pt-2">Total Budget</label>
                <div class="flex-1">
                  <input
                    type="number"
                    :value="creative.total_budget || ''"
                    @blur="(e) => updateField('total_budget', parseFloat((e.target as HTMLInputElement).value) || null)"
                    placeholder="0.00"
                    step="0.01"
                    class="w-full px-3 py-2 bg-gray-900/50 border border-gray-800 rounded-lg text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-gray-700"
                  />
                </div>
              </div>
            </div>

            <!-- Copy -->
            <div class="flex items-start gap-4">
              <label class="w-32 text-sm text-gray-400 pt-2">Ad Copy</label>
              <div class="flex-1">
                <textarea
                  :value="creative.ad_copy || ''"
                  @blur="(e) => updateField('ad_copy', (e.target as HTMLTextAreaElement).value)"
                  placeholder="Ad copy text..."
                  class="w-full px-3 py-2 bg-gray-900/50 border border-gray-800 rounded-lg text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-gray-700 resize-none"
                  rows="4"
                />
              </div>
            </div>

            <!-- Headline -->
            <div class="flex items-start gap-4">
              <label class="w-32 text-sm text-gray-400 pt-2">Headline</label>
              <div class="flex-1">
                <input
                  type="text"
                  :value="creative.headline || ''"
                  @blur="(e) => updateField('headline', (e.target as HTMLInputElement).value)"
                  placeholder="Ad headline..."
                  class="w-full px-3 py-2 bg-gray-900/50 border border-gray-800 rounded-lg text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-gray-700"
                />
              </div>
            </div>

            <!-- CTA -->
            <div class="flex items-start gap-4">
              <label class="w-32 text-sm text-gray-400 pt-2">Call to Action</label>
              <div class="flex-1">
                <input
                  type="text"
                  :value="creative.call_to_action || ''"
                  @blur="(e) => updateField('call_to_action', (e.target as HTMLInputElement).value)"
                  placeholder="e.g., Shop Now, Learn More"
                  class="w-full px-3 py-2 bg-gray-900/50 border border-gray-800 rounded-lg text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-gray-700"
                />
              </div>
            </div>

            <!-- Landing Page -->
            <div class="flex items-start gap-4">
              <label class="w-32 text-sm text-gray-400 pt-2">Landing Page</label>
              <div class="flex-1">
                <input
                  type="url"
                  :value="creative.landing_page_url || ''"
                  @blur="(e) => updateField('landing_page_url', (e.target as HTMLInputElement).value)"
                  placeholder="https://..."
                  class="w-full px-3 py-2 bg-gray-900/50 border border-gray-800 rounded-lg text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-gray-700"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </Transition>
</template>

<script setup lang="ts">
interface Props {
  modelValue: boolean
  creative?: any
}

const props = defineProps<Props>()
const emit = defineEmits<{
  (e: 'update:modelValue', value: boolean): void
  (e: 'updated'): void
}>()

const { supabase } = useSupabase()

// Update field function
const updateField = async (fieldName: string, value: any) => {
  if (!props.creative?.id) return
  
  try {
    const { error } = await supabase
      .from('creatives')
      .update({ 
        [fieldName]: value,
        updated_at: new Date().toISOString()
      })
      .eq('id', props.creative.id)
    
    if (error) throw error
    
    emit('updated')
    
    console.log(`✅ Updated ${fieldName}`)
  } catch (error) {
    console.error(`❌ Failed to update ${fieldName}:`, error)
  }
}
</script>

<style scoped>
.slide-enter-active,
.slide-leave-active {
  transition: all 0.3s ease;
}

.slide-enter-from,
.slide-leave-to {
  transform: translateX(100%);
}

.slide-enter-from .absolute,
.slide-leave-to .absolute {
  opacity: 0;
}

.slide-enter-active .absolute,
.slide-leave-active .absolute {
  transition: opacity 0.3s ease;
}
</style>
