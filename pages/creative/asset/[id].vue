  ...
  <div class="h-screen bg-[#1E1E1E] flex flex-col text-white">
    ...
    <div class="bg-[#2D2D2D] border-b border-gray-800 px-6 py-3 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3 text-sm">
        <button @click="$router.back()" class="hover:bg-gray-700 p-2 rounded transition-colors">
        ...
        <span class="text-gray-600">/</span>
        <span class="text-white">{{ asset?.title || 'Glute Burner Circuit' }}</span>
      </div>
      ...
      <div class="flex items-center gap-3">
        <button class="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
          ...
          <span>{{ asset?.reviewCount || 33 }} Needs Review</span>
        </button>
        ...
        <button @click="$router.back()" class="p-2 hover:bg-gray-700 rounded transition-colors">
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <div class="w-96 bg-[#2D2D2D] border-l border-gray-800 flex flex-col">
        ...
        <div v-if="activeTab === 'info'" class="flex-1 overflow-y-auto p-4 space-y-4">
          <div>
            <h2 class="text-lg font-semibold mb-2">{{ asset?.title || 'Glute Burner Circuit' }}</h2>
            <div class="flex items-center gap-2 text-sm text-gray-400">
              <span>V1</span>
              <span>·</span>
              <span>{{ asset?.format || 'MOV' }}</span>
              <span>·</span>
              <span>{{ asset?.size || '47 MB' }}</span>
              <span>·</span>
              <span>{{ asset?.dimensions || '1080 × 1920' }}</span>
              <span>·</span>
              <span>{{ asset?.duration || '0:36' }}</span>
            </div>
          </div>
